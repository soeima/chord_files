...
class Draggable extends React.Component {
    ...
    handleTouchStart = e => {
        ...
        this.setState({
            activeDrag: true,
            translateX: e.touches[0].clientY
        })
    }

    handleMouseMove = event => {
        var translateX =  event.clientY - this.state.translateX
        var newX = this.state.posX - translateX
        if(newX < 0) newX = 0
        else if (newX > this.props.width) newX = this.props.width
        this.setState({
            posX: newX,
            translateX: event.clientY
        })
    ...
    handleTouchMove = event => {
        var translateX =  event.touches[0].clientY - this.state.translateX
        var newX = this.state.posX - translateX
        if(newX < 0) newX = 0
        else if (newX > this.props.width) newX = this.props.width
        this.setState({
            posX: newX,
            translateX: event.touches[0].clientY
        })
